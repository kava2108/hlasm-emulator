...
### 8-8. 実務寄りのサンプル追加、DC C型の右パディングのバグ修正

ユーザーから「実際の実務的なhlasmはありませんか」と依頼され、
`examples/order_batch.hlasm`を追加。受注明細の**固定長レコードテーブル**
（商品コードCL8+数量PL8+単価PL8+金額PL8+フラグCL1＝33バイト/件）を
ループ処理し、数量×単価を`CVB`→`MR`→`CVD`（パックド10進数どうしを
直接掛けるMP命令は未対応のためバイナリ経由）で計算してレコードへ書き戻し、
しきい値超過に`H`フラグを立てるバッチ集計プログラム。
COBOL/HLASMの実務でよくある「固定長レコードをテーブル化してループで
処理する」パターンを意識した。

**このサンプル作成中に見つけた既存のバグ2件**:
1. **オペランド間の余分な空白でDCのフィールドが欠落する**: 見た目を
   揃えるためカンマの後に空白を入れたところ（`PL8'5', PL8'980'`）、
   HLASMの構文では引用符外の空白がオペランドフィールドの終端（以降は
   remarks扱い）になるため、hlasm-parserがそこで残りのオペランドを
   切り捨てていた。**これはhlasm-parser側の正しい仕様**（本物のHLASM
   もそうなる）であり、単にこちらのサンプルの書き方が誤っていた
   （空白を除去して解決）
2. **`dc_values.py`の実バグ**: `DC CL8'GIZMO-C'`のように文字定数が
   宣言長より短い場合、数値型と同じ「左側をゼロバイトでパディング」を
   適用してしまい、`GIZMO-C`(7文字)が8バイト中で1バイトの`\x00`を
   先頭に持つ壊れた値になっていた。実際のHLASMでは文字定数は
   **左詰め・右側をEBCDIC空白でパディング**が正しい。これまでのテストは
   全て文字定数の長さが宣言長とちょうど一致するケースだけだったため
   検出されていなかった（`tests/test_dc_values.py`に回帰テスト3件を
   追加、`CL8'HI'`が正しく`"HI"`+空白6個になることを確認）

`tests/test_example_programs.py`に`order_batch.hlasm`の回帰テストを追加
（3レコード分の商品コード・金額・フラグとTOTALをPython側で再計算して
突き合わせ）。pytest全体で89件グリーン。
